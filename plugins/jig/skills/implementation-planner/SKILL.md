...
- **The developer changes a decision after its checklist exists.** The change goes through the spec, never into the checklist by hand: write it into the spec with its `Developer said:` line, then re-run this skill on the spec. Rewrite the checklist that the changed text covers, in place — same file, same number, same tracker row and Status — and run Step 5 on it. A checklist edited outside this skill skipped Step 3 and Step 5: its new rows carry no `Developer said:` line and no recipe walk, so a builder finds the gaps at build time and fills them itself.
