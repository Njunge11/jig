...
11. No `## Manual verification` item names a script the repo can run, except the eval scripts, which stay there. Any other one moves to `## Done`, with the task that makes the script load its own keys when it needs them.
