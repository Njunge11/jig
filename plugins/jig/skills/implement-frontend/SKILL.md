...
2. **Restate the UI.** You usually get it as prose, sometimes with screenshots or mockups. When images come with the work, they are the layout, functionality, and flow — read every one, and open the restatement with one paragraph per image saying what it shows. The anatomy must match the images; only the design tokens come from the codebase's design system, never from the mockup's colors. If the work references an image you cannot see, stop and say so — never restate or build a design you did not see. Then write the page's anatomy — a named tree of parts, one responsibility per part, one owner per piece of shared state — before any code. The preloaded `structure` skill places every file this lane produces; never place a file by imitating existing code. **Gate:** the restatement names each provided image and what it shows, or states that none were provided.
