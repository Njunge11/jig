...
1. **Create the feature branch before any commit**: `git checkout -b <type>/<feature-slug>` off an up-to-date `main`. Use the branch names from the preloaded `open-feature-pr` skill. If the session is already on the branch of this feature, continue on that branch. When the checklist has a `## Backend` section, the `implement-backend` run already created and pushed the branch named in the checklist's Scope: `git fetch origin` and `git checkout <branch>`, and build on it. Never commit feature work to `main` or an unrelated branch. **Gate:** `git branch --show-current` prints the feature branch, not `main`.
2. **Restate the UI.** You usually get it as prose, sometimes with screenshots or mockups. When images come with the work, they are the layout, functionality, and flow — read every one, and open the restatement with one paragraph per image saying what it shows. The anatomy must match the images; only the design tokens come from the codebase's design system, never from the mockup's colors. If the work references an image you cannot see, stop and say so — never restate or build a design you did not see. When the checklist has a `## Design facts` section, it is the transcription of those images — build to its statements. When images come with the work and the checklist has no `## Design facts` section, author it first: one `D<n>` statement per checkable fact, grouped by image — parts, position, order, counts, alignment, variants, copy word for word; never a pixel size or a color read off the image. Then write the page's anatomy — a named tree of parts, one responsibility per part, one owner per piece of shared state — before any code. The preloaded `structure` skill places every file this lane produces; never place a file by imitating existing code. **Gate:** the restatement names each provided image and what it shows, or states that none were provided; and a `## Design facts` section exists whenever images were provided.
3. **Name the recipes that apply.** Every `recipe-*` skill is preloaded, so every recipe is already in your context — you load nothing. Walk the catalog of the preloaded `frontend-standards` skill row by row against every `F<n>` and `V<n>` item of the checklist, and record one verdict per row: `applies` with the item numbers, or `does not apply`. A recipe that the checklist names does not end the walk. Follow every recipe that applies — the recipe orders the construction. When an item describes a behavior that the Don't list of an applying recipe forbids, the recipe wins: build to the recipe and record the difference in the handoff's **Deviations**. **Gate:** every catalog row has a recorded verdict.
4. **The `## Frontend + Integration` section of the checklist is the task list.** If the checklist does not have that section, author it first, split into **Behavior (test-backed)** `F<n>` items and **Visual & responsive (browser-checked, never jsdom tests)** `V<n>` items. Cover error and empty states, not just the happy path. Preserve described copy verbatim; never invent an undecided screen, state, or label — record the open question in the handoff's **Deviations**. Work through the `F<n>` items with [the loop](#the-loop) below. **Gate:** every `F<n>` item is `[x]`, and the test file of each task is green.
...
6. **Check the `## Rules` list of the preloaded `frontend-standards` skill, plus the Verify list of every recipe that step 3 recorded as `applies`, item by item against your diff** (`git diff main`). When the checklist has a `## Design facts` section, walk it the same way: one verdict per `D<n>` statement against the diff — `pass`, `fixed`, or `browser-check` when only the rendered page can settle it. When the diff touches a client that imports `eve/react`, check the Review checklist of the preloaded `eve-agent` skill the same way and run its Gates. When the diff touches a file that imports `xstate` or `@xstate/react`, check the Shared and Frontend items of the Review checklist of the preloaded `state-machines` skill the same way and run its Gates for Part A and Part C. Walk the tree and the Placement rules of the preloaded `structure` skill over every file the diff adds or moves, and move a misplaced file now. Name each item that the diff violates. Fix each violation. Commit the fixes as one commit. **Gate:** every item and every added or moved file has a recorded verdict and `vitest` is green. This is the one full run of the suite in this workflow.
7. **Visual & responsive.** Verify each `V<n>` item in the browser at 375, 768, 1024, and 1440, and tick it. A `D<n>` fact recorded `browser-check` in step 6 is settled in the same browser pass. When you cannot run a browser, leave the item unticked and list it in the proof as `browser-check`. Never tick a `V<n>` item from the code alone. **Gate:** every `V<n>` item is ticked or listed as `browser-check`.
8. **Write the handoff and flip the tracker.** Write `docs/<project>/handoffs/NN-<slug>.md` — the `handoffs/` folder sits beside the checklist's folder. Use the [handoff format](#handoff-format) below. When the backend run of the same checklist already wrote that file, append your lines to its three sections instead of writing a second file. Then set this checklist's Status to `Done` in `docs/<project>/tracker.md`. **Gate:** both files exist on the branch and are committed.
...
recipes
 recipe-page-with-data applies — F1 F2
 recipe-data-table applies — F3
...
review   <paste the verdict the review-frontend-feature run returned: one line per item, plus the test run of the files it fixed, when it fixed one>
```
...
1. **Red.** Write one real, runnable test for the task, driven through what the user sees and does — MSW at the network edge, or seed the cache. Run the test to see it fail. Run that test file by name (`vitest run <file>`), never the full suite: the full suite runs one time, at the gate of the standards walk. A test that passes before the implementation exists does not test the new behavior.
...
2. Walk the `## Rules` of the preloaded `frontend-standards` skill, plus the Verify list of every preloaded recipe whose catalog row matches, over the existing surface; record every violation with `file:line`.
