...
- **TDD implementation checklists:** every task in every section is `[x]`. The suite is green. The file `docs/<project>/handoffs/NN-<slug>.md` exists. The tracker Status is flipped. A checklist with both a `## Backend` and a `## Frontend + Integration` section gets one PR, opened by the frontend run; the backend run only pushes the branch.
