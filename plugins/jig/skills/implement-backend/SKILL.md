...
2. The `## Backend` section of the checklist is the task list. Work through it with [the loop](#the-loop) below. **Gate:** every `## Backend` item is `[x]`, and the test file of each task is green.
3. **Check the Review checklist of the preloaded `backend-standards` skill item by item against your diff** (`git diff main`). Name each item that the diff violates. Fix each violation. When the diff touches `agent/` or `evals/`, check the Review checklist of the preloaded `eve-agent` skill the same way and run its Gates. When the diff touches a file that imports `xstate` or a file under a `machine/` folder, check the Shared and Backend items of the Review checklist of the preloaded `state-machines` skill the same way and run its Gates for Part A and Part B. Walk the tree and the Placement rules of the preloaded `structure` skill over every file the diff adds or moves, and move a misplaced file now. Commit the fixes as one commit. **Gate:** every item and every added or moved file has a recorded verdict and `vitest --project backend` is green. This is the one full run of the suite in this workflow.
4. **Write the handoff and flip the tracker.** Write `docs/<project>/handoffs/NN-<slug>.md` — the `handoffs/` folder sits beside the checklist's folder. Use the [handoff format](#handoff-format) below. Then set this checklist's Status to `Done` in `docs/<project>/tracker.md`. When the checklist also has a `## Frontend + Integration` section, the slice is not done yet: write the handoff for the Backend items, set the Status to `In progress`, and leave `Done` to the frontend run. **Gate:** both files exist on the branch and are committed.
5. **Open the PR as the preloaded `open-feature-pr` skill specifies.** That skill owns the branch, title, and body format, and the `gh` steps. Do not make your own format. **Gate:** `gh` returns the PR URL. When the checklist also has a `## Frontend + Integration` section, open no PR: run `git push -u origin HEAD` and stop there — the `implement-frontend` run on the same checklist opens the one PR for the slice. **Gate:** the push output names the branch on `origin`.
...
PR       https://github.com/<org>/<repo>/pull/<n> — or, when the checklist has a Frontend + Integration section: pushed <branch>; the PR opens with the implement-frontend run
review   <paste the verdict the review-backend-feature run returned: one line per item, plus the test run of the files it fixed, when it fixed one>
```
...
1. **Red.** Write one real, runnable test for the task. Run the test to see it fail. Run that test file by name (`vitest run <file>`), never the full suite: the full suite runs one time, at the gate of the standards walk. A test that passes before the implementation exists does not test the new behavior.
