...
3. **Check the Review checklist of the preloaded `backend-standards` skill item by item against your diff** (`git diff main`). Name each item that the diff violates. Fix each violation. When the diff touches `agent/` or `evals/`, check the Review checklist of the preloaded `eve-agent` skill the same way and run its Gates. Commit the fixes as one commit. **Gate:** every item has a recorded verdict and `vitest --project backend` is green.
