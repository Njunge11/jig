...
| 2 | Gate 10 grep counts an untracked stale artifact (`.tend/report.json`)      | Same initializer bug               | **Landed 2026-08-09:** run-every-gate-before-writing-it rule added; tracked-file assertions must use `git grep`. |
| 3 | Evaluator loop over output verbosity; full 1,762-test output demanded      | Rule ambiguous                     | **Landed 2026-08-09:** the chore-lane /goal template now defines proof as command + final summary + exit status. |
| 4 | Done checkboxes never ticked while tracker says Done                       | Rule missing                       | **Landed 2026-08-09:** builder ticks each box when its output is pasted. |
